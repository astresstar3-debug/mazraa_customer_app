...
class _ReferenceBottomBar extends StatelessWidget {
  const _ReferenceBottomBar({required this.index, required this.cartCount, required this.onChanged});
  final int index;
  final int cartCount;
  ...
  Widget build(BuildContext context) {
    ...
    return Material(
      ...
      child: SafeArea(
        ...
        child: SizedBox(
          height: 67,
          child: Row(children: [for (var i = 0; i < items.length; i++) Expanded(child: InkWell(onTap: () => onChanged(i), child: _NavItem(active: index == i, icon: index == i ? items[i].selected : items[i].icon, label: items[i].label, badge: i == 3 ? cartCount : 0)))]),
        ),
      ),
    );
  }
...
class _NavItem extends StatelessWidget {
  const _NavItem({required this.active, required this.icon, required this.label, required this.badge});
  final bool active;
  final IconData icon;
  final String label;
  final int badge;
  ...
  Widget build(BuildContext context) {
    ...
    return Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      SizedBox(height: 28, child: Stack(clipBehavior: Clip.none, alignment: Alignment.center, children: [
        Icon(icon, color: color, size: active ? 26 : 24),
        if (badge > 0) PositionedDirectional(top: -5, end: -12, child: Container(constraints: const BoxConstraints(minWidth: 18, minHeight: 18), padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 2), decoration: const BoxDecoration(color: AppColors.terracotta, shape: BoxShape.circle), alignment: Alignment.center, child: Text('$badge', style: const TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.w900)))),
      ])),
      const SizedBox(height: 3),
      Text(label, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: color, fontSize: 9.5, fontWeight: active ? FontWeight.w800 : FontWeight.w500)),
    ]);
  }
}
